...
categorical_pipeline = Pipeline([("one-hot encoding", one_hot_encoder)])

numerical_pipeline = Pipeline([("impute", median_imputer), ("normalization", min_max_scaler)])
...
def assess_regression_model(model, X_train, X_test, y_train, y_test) -> tuple[float, float]:
    ...
    # your_code_here
    rmse_pred = root_mean_squared_error(y_train, train_pred)
    rmse_test = root_mean_squared_error(y_test, test_pred)
    
    # print train and test RMSE
    # your_code_here
    print(f"Train RMSE: {rmse_pred:.2f}$")
    print(f"Test RMSE: {rmse_test:.2f}$")
    print(f"Rożnica wynosi: {abs((rmse_pred-rmse_test)/rmse_test)*100:.2f}%")

    return (rmse_pred, rmse_test)
    # your_code
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Błąd na danych testowych jest o 20% większy niż na danych treningowych sugeruje to overfitting modelu. Biorąc pod uwagę średnią cenę wynoszącą około 180k$ wydaje mi się, że nie jest to duży błąd.
#
...
reg_ridge_cv = RidgeCV(alphas, cv = None)
...
print(f"{reg_ridge_alpha:.4f}")
print()

print("Dla Lasso")
lasso_train_rmse, lasso_test_rmse = assess_regression_model(reg_lasso_cv, X_train, X_test, y_train, y_test)
print(f"{reg_lasso_alpha:.4f}")
print()

#ciekawostka
print("Dla Elastica")
reg_elastic_cv = ElasticNetCV(l1_ratio = np.linspace(0.1, 1, 21), alphas = alphas, max_iter = 10000, cv = 5, random_state = 0)
reg_elastic_cv.fit(X_train, y_train)
reg_elastic_alpha = reg_elastic_cv.alpha_

assess_regression_model(reg_elastic_cv, X_train, X_test, y_train, y_test)
print(f"{reg_elastic_alpha:.4f}")
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# W przypadku Ridge udało się zmniejszyć overfitting ponad dwukrotnie, w przypadku Lassa udało się go zmniejszyć do zaledwie 3%, ale niestety było spowodowane zwiększeniem się błędu na danych treningowych co przybliżyło jego wartość do błędu na danych testowych.
#
#
...
# nie ma żadnych indexów więc df_nan jest pusty czyli wszędzie są wartośći
...
ax.set_xlabel("Class")
ones = sum(1 for r in y if r)
zeros = len(y) - ones
print(f"Wystepowanie 0: {(zeros/(zeros+ones))*100:.2f}%", end=" ")
print(f"Wystepowanie 1: {(ones/(zeros+ones))*100:.2f}%")
...
# %% editable=true slideshow={"slide_type": ""} tags=["ex"]
# Mogło to być wcześniej importowane, ale wole żeby wszystko było w jednym miejsu
from sklearn.compose import ColumnTransformer
from sklearn.impute import SimpleImputer
from sklearn.pipeline import Pipeline
...
print(f"Precyzja: {nol_precision:.3f}, Recall: {nol_recall:.3f}, F1_Score: {nol_f1:.3f}")
print()
#2 regularyzacją L2 za pomocą LogisticRegressionCV
logistic_regression_cv = LogisticRegressionCV(Cs=100, cv=5, scoring="f1", class_weight="balanced", n_jobs=-1, random_state=0)
...
print("Regularyzacja L2 za pomocą LogisticRegressionCV")
print(f"Precyzja: {l2_precision:.3f}, Recall: {l2_recall:.3f}, F1_Score: {l2_f1:.3f}")
print()
#3 regularyzacją L1 za pomocą LogisticRegressionCV i solvera Saga
logistic_regression_cv_l1 = LogisticRegressionCV(Cs=100, cv=5, penalty="l1", scoring="f1", solver="saga", class_weight="balanced", n_jobs=-1, random_state=0)
logistic_regression_cv_l1.fit(X_train, y_train)

y_pred = logistic_regression_cv_l1.predict(X_test)
l1_precision = precision_score(y_test, y_pred)
l1_recall = recall_score(y_test, y_pred)
l1_f1 = f1_score(y_test, y_pred)
print("Regularyzacja L1 za pomocą LogisticRegressionCV i solvera Saga")
print(f"Precyzja: {l1_precision:.3f}, Recall: {l1_recall:.3f}, F1_Score: {l1_f1:.3f}")
...
y_train_pred = logistic_regression.predict(X_test)
f1_test = f1_score(y_test, y_train_pred)

y_test_pred = logistic_regression.predict(X_train)
f1_train = f1_score(y_train, y_test_pred)

print(f"Precyzja F1 na danych Testowych: {f1_test:.3f}, Precyzja F1 na danych Treningowych: {f1_train:.3f}")
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Wydaje mi się, że precyzja jest istotniejsza jako, że bank chce dotrzeć dotrzeć przedewszystkim do tych którzy już są zainteresowani reklamą i faktycznie założą lokatę i nie chce on zrażać potencjalnych klientów zbyt dużą ilością reklam. Jeżeli chodzi o stosunek precision/recall moglibyśmy go zmienić zmieniając treshhold tj. przesunąć granicę w, której model uznaje przykład za pozytywny, domyślnie jest to 0.5, ale można go zmienić na 0.2 lub 0.8 manipulując w ten sposób ilością zwracanych pozytywów.
#
...
# %% editable=true slideshow={"slide_type": ""} tags=["ex"]
#większość z tych rzeczy była już zdefiniowania ale wrzuca, dla przypomnienia, aby wszystko było w jednym miejsxu
from sklearn.compose import ColumnTransformer
from sklearn.impute import SimpleImputer
from sklearn.pipeline import Pipeline
...
numerical_pipeline = Pipeline([("standard_scaler", standard_scaler), ("polynomial_features", polynomial_features)])
...
logistic_regression_pol = LogisticRegression(penalty=None, class_weight="balanced", max_iter=2000)
logistic_regression_pol.fit(X_train, y_train)

y_train_pred = logistic_regression_pol.predict(X_test)
f1_test = f1_score(y_test, y_train_pred)

y_test_pred = logistic_regression_pol.predict(X_train)
f1_train = f1_score(y_train, y_test_pred)
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Czy jest sens tworzyć modele z regularyzacją? Myślę, że zależy to od danych do których, na których trenujemy model. W tym przypadku regularyzacja nie jest potrzebna, otrzymywane tezultaty są niemal idetyczne, a regularyzacja zabiera dodatkowy czas. W tym zadaniu także inny preprocessing danych poskutkował lepszym modelem niż regularyzacja.
#
